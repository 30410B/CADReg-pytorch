...
import numpy as np
import LPBA_CADReg_pytorch.Models.configs_Trans as configs
...
class ConvInsBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.activation(out)
        return out
class FEEncoder(nn.Module):
    """
    Main model
    """

    def __init__(self, in_channel=1, first_out_channel=4):
        super(FEEncoder, self).__init__()
...
class CNATransformer(nn.Module):
    ...
    def forward(self, q, k):
        ...
        k = k.permute(0, 4, 1, 2, 3)  # B, C, H, W, T
        ...
        #x=x.reshape(B, H, W, T, self.num_heads,3)
...
class DeformHead_block(nn.Module):
    ...
    def forward(self, x_in):
        ...
        return x_out
class CADReg(nn.Module):
    def __init__(self, config,head_dim=6,num_heads=[8, 4, 2],scale=1):
        '''
        CADReg Model
        '''
        super(CADReg, self).__init__()
        ...
        c=4
        self.up0 = DecoderBlock(embed_dim*8, embed_dim*4, skip_channels=embed_dim*4+64*c if if_transskip else 0, use_batchnorm=False)
        self.up1 = DecoderBlock(embed_dim*4, embed_dim*2, skip_channels=embed_dim*2+32*c if if_transskip else 0, use_batchnorm=False)  # 384, 20, 20, 64
        self.up2 = DecoderBlock(embed_dim*2, embed_dim, skip_channels=embed_dim+16*c if if_transskip else 0, use_batchnorm=False)  # 384, 40, 40, 64
        self.up3 = DecoderBlock(embed_dim, embed_dim//2, skip_channels=8*c if if_convskip else 0, use_batchnorm=False)  # 384, 80, 80, 128
        self.up4 = DecoderBlock(embed_dim//2, config.reg_head_chan, skip_channels=4*c if if_convskip else 0, use_batchnorm=False)  # 384, 160, 160, 256
        ...
        self.feencoder = FEEncoder(in_channel=1, first_out_channel=c)


        self.projblock3 = ProjectionLayer(8*c, dim=head_dim*num_heads[2])
        self.mdt3 = CNATransformer(head_dim*num_heads[2], num_heads[2], qk_scale=scale)
        self.cwm3 = CWM(3 * num_heads[2], 3 * num_heads[2] * 2)

        self.projblock4 = ProjectionLayer(16*c, dim=head_dim*num_heads[1])
        self.mdt4 = CNATransformer(head_dim*num_heads[1], num_heads[1], qk_scale=scale)
        self.cwm4 = CWM(3 * num_heads[1], 3 * num_heads[1] * 2)

        self.projblock5 = ProjectionLayer(32*c, dim=head_dim*num_heads[0])
        self.mdt5 = CNATransformer(head_dim*num_heads[0], num_heads[0], qk_scale=scale)
        self.cwm5 = CWM(3 * num_heads[0], 3 * num_heads[0] *2)

        self.projblock6 = ProjectionLayer(32*c, dim=head_dim*num_heads[0])
        self.mdt6 = CNATransformer(head_dim*num_heads[0], num_heads[0], qk_scale=scale)
        self.cwm6 = CWM(3 * num_heads[0], 3 * num_heads[0] *2)



        self.reghead_1 = DeformHead_block(embed_dim*8, use_checkpoint=True)
        self.reghead_2 = DeformHead_block(embed_dim*4, use_checkpoint=True)
        self.reghead_3 = DeformHead_block(embed_dim*2, use_checkpoint=True)
        self.reghead_4 = DeformHead_block(embed_dim*1, use_checkpoint=True)
        self.reghead_5 = DeformHead_block(embed_dim//2, use_checkpoint=True)
        self.sptransformer = nn.ModuleList()
        for i in range(6):
            self.sptransformer.append(SpatialTransformer([s // 2**i for s in config.img_size]))     
    def forward(self, x):
        source = x[:, 0:1, :, :]
        fixed = x[:, 1:2, :, :]
        M1, M2, M3, M4, M5, M6 = self.feencoder(source)
        F1, F2, F3, F4, F5, F6 = self.feencoder(fixed)
        ...
        q6, k6 = self.projblock6(F6), self.projblock6(M6)
        w = self.mdt6(q6, k6)
        atten6 = self.cwm6(w)          
        flow=[None]*6
        flow6=self.reghead_1(out_feats[-1])
        flow6=flow6+atten6
        flow[5]=flow6

        M5_new=self.sptransformer[4](M5,self.upsample(flow6))
        q5, k5 = self.projblock5(F5), self.projblock5(M5_new)
        w = self.mdt5(q5, k5)
        atten5 = self.cwm5(w)
        f1=torch.cat((M5_new,F5,f1),dim=1)
        x1 = self.up0(out_feats[-1], f1)
        flow5=self.reghead_2(x1)
        flow5=flow5+atten5
        flow5=flow5+self.upsample(flow6)
        flow[4]=flow5

        M4_new=self.sptransformer[3](M4,self.upsample(flow5))
        q4,k4 = self.projblock4(F4), self.projblock4(M4_new)
        w=self.mdt4(q4, k4)
        atten4 = self.cwm4(w)
        f2=torch.cat((M4_new,F4,f2),dim=1)           
        x2 = self.up1(x1, f2)
        flow4=self.reghead_3(x2)
        flow4=flow4+atten4
        flow4=flow4+self.upsample(flow5)     
        flow[3]=flow4

        M3_new=self.sptransformer[2](M3,self.upsample(flow4))
        q3, k3 = self.projblock3(F3), self.projblock3(M3_new)
        w = self.mdt3(q3, k3)
        atten3 = self.cwm3(w)
        f3=torch.cat((M3_new,F3,f3),dim=1)        
        x3 = self.up2(x2, f3)
        flow3=self.reghead_4(x3)
        flow3=flow3+atten3
        flow3=flow3+self.upsample(flow4)  
        flow[2]=flow3

        M2_new = self.sptransformer[1](M2,self.upsample(flow3))
        if self.if_convskip:
            f4 = torch.cat((M2_new,F2), dim=1)
            #f5 = torch.cat(M1,F1)
        else:
            f4 = None
            #f5 = None
        x4 = self.up3(x3, f4)
        flow2=self.reghead_5(x4)+self.upsample(flow3)
        flow[1]=flow2

        M1_new = self.sptransformer[0](M1,self.upsample(flow2))
        if self.if_convskip:
            #f4 = torch.cat(M2_new,F2)
            f5 = torch.cat((M1_new,F1), dim=1)
        else:
            #f4 = None
            f5 = None
        x5 = self.up4(x4,f5)
        flow1=self.reg_head(x5)+self.upsample(flow2)
        flow[0]=flow1
        #out = self.spatial_trans(source, flow1)
        return flow1
