...
def train():
    #结果文件
    expdir= 'Version'+str(args.n_iter) + "_" + str(args.lr) + "_" + str(args.alpha)+"/"
    make_dirs()
    device = torch.device('cuda:{}'.format(args.gpu) if torch.cuda.is_available() else 'cpu')

    # 日志文件
    log_name = 'Version'+str(args.n_iter) + "_" + str(args.lr) + "_" + str(args.alpha)
    ...
    writer = SummaryWriter(log_dir='Log/'+expdir)
    # Training loop.
    for i in range(iterEpoch, args.n_iter ):
        ...
        for input_moving, fig_name in input_moving_all:
            adjust_learning_rate(opt, i, args.n_iter, args.lr)
            # [B, C, D, W, H]
            fig_name = fig_name[0]
            ...
            x=torch.cat((input_fixed,input_moving),dim=1)
            flow_m2f = net(x)
            ...
            y=torch.cat((input_moving,input_fixed),dim=1)
            flow_m2f = net(y)
        ...
        for file in test_file_lst:
            ...
            # 获得配准后的图像和label
            inputimg=torch.cat((input_fixed_eval,input_moving),dim=1)
            pred_flow = net(inputimg)
        ...
        writer.add_scalar('DSC/TEST', np.mean(DSC), i)
        writer.add_scalar('STD/TEST', np.std(DSC), i)
        save_checkpoint({
            'epoch': i,
            'state_dict': net.state_dict(),
            'optimizer': opt.state_dict(),
            }, save_dir='experiments/'+expdir, filename='dsc{:.4f}epoch{:0>3d}.pth.tar'.format(np.mean(DSC), i))
